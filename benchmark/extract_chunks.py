...
def extract(pdf_dir: Path, out_dir: Path, work_dir: Path, grobid_url: str, timeout: int):
    ...
    for pdf in sorted(pdf_dir.glob("*.pdf")):
        ...
        try:
            chunks, _ = processor.extract_text_chunks(pdf.stem)
        except (ValueError, OSError) as error:
            failed.append((pdf.stem, str(error)))
            continue
        target.write_text(json.dumps([p.text for p in chunks]))
    return failed
...
def main():
    ...
    failed = extract(Path(args.pdf_dir), Path(args.out_dir), Path(args.work_dir),
